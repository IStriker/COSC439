...
int main(int argc, char *argv[])
{
    ...
    while(TRUE){
        ...
        if(strcmp(recieve_message.message, "Login success") == 0 && send_message.request_type != Logout){
...
/**
 * asks the user for the leader they want to 
 * request to see their feeds
 * @return leader id
 */
 unsigned int request_message_feeds(){
...
start_request:;
	 unsigned int id;
	 printf("Please enter id of leader: ");
	 scanf("%d", &id);
	 
	 //check if id exist in leaders list
	 int i;
	 for(i = 0; i < 10; i++){
		 if(sample_leaders_ids[i] == id){
			 return id;
		 }
	 }
...
start_unfollow:;
	 ...
	 printf("Please enter id of leader you want to unfollow: ");
	 scanf("%d", &id);
